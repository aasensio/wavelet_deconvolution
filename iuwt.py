...
def starlet_transform_pt(input_image, num_bands = None, gen2 = True):
    '''
    Computes the starlet transform of an image (i.e. undecimated isotropic
    wavelet transform).
    ...
    for band in range(num_bands):
        im_out = bspline_star_pt(im_in, step_trou)
        if gen2:  # Gen2 starlet applies smoothing twice
            WT.append(im_in - bspline_star_pt(im_out, step_trou))
        else:
            test = im_in - im_out
            WT.append(im_in - im_out)
        im_in = im_out
...
if __name__ == '__main__':
    ...
    out = starlet_transform(image, num_bands = 3, gen2 = True)

    out_pt = starlet_transform_pt(image_pt, num_bands = 3, gen2 = True)

    fig, ax = pl.subplots(nrows=3, ncols=4, figsize=(20, 15))
    for i in range(4):
        ax[0, i].imshow(out[i][0, 0, :, :])
        ax[1, i].imshow(out_pt[i].cpu().numpy()[0, 0, :, :])
        ax[2, i].imshow(out[i][0, 0, :, :] - out_pt[i].cpu().numpy()[0, 0, :, :])